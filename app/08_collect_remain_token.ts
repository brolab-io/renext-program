...
export async function collectRemainToken(
    ...
    console.log(`Creator ${creator.publicKey.toBase58()} want collect ${pool_info.poolSizeRemaining.toNumber()} token ${mint.toBase58()} remain in pool ${launch_pool.toBase58()}`);
    console.log('--------------------------------------')
    ...
    console.log("Collect remain token in tx: ", '\n', getExplorerTxUrl(tx));
    console.log('********************************')
