use crate::{
    ...
    errors::MyError,
    events::RemainTokenCollectedEvent,
...
#[derive(Accounts)]
pub struct CollectRemainToken<'info> {
    #[account(
        mut,
...
pub fn handler(ctx: Context<CollectRemainToken>) -> ProgramResult {
    let launch_pool = &mut ctx.accounts.launch_pool;

    require!(
        launch_pool.status.eq(&LaunchPoolState::Completed)
            || launch_pool.status.eq(&LaunchPoolState::Cancelled),
    ...
    emit!(RemainTokenCollectedEvent {
